...
*/
	...
	// require all esri, dojo, and dijit libraries 
	...
	MERI_MunicipalMap.prototype.loadDependencies = function( ) {
		...
		var requires = ["esri.map", "esri.tasks.geometry", "esri.toolbars.navigation", "esri.dijit.Scalebar", "esri.tasks.query", "dijit.form.Button", "dijit.Toolbar", "dojo.fx", "esri.dijit.OverviewMap", "esri.dijit.Measurement"];
		dojo.forEach( requires, function( r ){ dojo.require( r );} );

		// request various layer defs.. wait for all to load before continuing
		dojo.xhrGet({
	...
	MERI_MunicipalMap.prototype.loadContinue = function( ) {
		...
		function f_layer_list_build() {
			...
			dojo.forEach( self.map_layers_json, function( group, index ){
		...
		function f_imagery_list_build( layer_selected ){
			...
			dojo.forEach( self.imagery_layers_json, function( img_lyr, index ){
	...
	/////////////////
		...
		function mapInitialize( aliasObj ) {
		...
		// initialize all query functionality
