...
<script type="text/javascript" src="http://webmaps.njmeadowlands.gov/municipal/js/MERI_formatting.js"></script>
...
*/
	...
	// require all esri, dojo, and dijit libraries efficiently
	dojo.require("esri.map"); dojo.require("esri.tasks.geometry"); dojo.require("esri.toolbars.navigation"); dojo.require("esri.dijit.Scalebar"); dojo.require("esri.tasks.query"); 
	
	dojo.require("dijit.form.Button"); dojo.require("dijit.Toolbar"); dojo.require("dojo.fx"); dojo.require("esri.dijit.OverviewMap");dojo.require("esri.dijit.Measurement");

	// map globals;
	var M_meri, basemap_dynamic, navToolbar;
	var LD_base, LD_visible = [];
	var LD_flooding, LD_flood_visible =[];
	var ERIS_base;

	// selected tool item. default to select
	var tool_selected = 'select';

	// initialize all query and query task objects
	// query tasks
	...
	// object for collection of selected parcels and buffered parels
	...
	// pull in json data source
	
	var map_legend = null;
	var map_legend_eris = null;
	
	// map layers... toc for layers is dynamically build from JSON object
	var map_layers_json = [		
		{name:'Environmental',id:'environ',layers:[	
	...
	var landuse_json =[
		{code:'000',lclass:'Unclassified'},
		{code:'AL',lclass:'Altered Lands'},
		{code:'CO',lclass:'Commercial Office'},
		{code:'CR',lclass:'Commercial Retail'},
		{code:'CU',lclass:'Communication Utility'},
		{code:'HM',lclass:'Hotels and Motels'},
		{code:'ICC',lclass:'Ind. Comm. Complex'},
		{code:'IND',lclass:'Industrial'},
		{code:'PQP',lclass:'Public Services'},
		{code:'RES',lclass:'Residential'},
		{code:'RL',lclass:'Recreational Land'},
		{code:'TRS',lclass:'Transportation'},
		{code:'VAC',lclass:'Vacant Land'},
		{code:'WAT',lclass:'Water'},
		{code:'WET',lclass:'Wetlands'}
	];
	...
	function f_layer_list_build() {
		...
		dojo.forEach(map_layers_json,function(group,index){
			...
			dojo.forEach(group.layers,function(layer,lyrIndex){
				...
				if(layer.vis){
					e_chk.setAttribute('checked', 'checked');
				}
	...
	dojo.forEach(map_layers_flooding_json.scenarios,function(scenario,index){
		...
		if(scenario.vis){
			e_chk_group.setAttribute('checked', 'checked');
		}
	...
	function f_layer_list_flood_update() {
		...
		dojo.forEach(inputs,function(input){
		  if (input.checked) {
			  LD_flood_visible.push(input.id.replace('m_layer_flood_',''));
		  }
		});

		//if there aren't any layers visible set the array to be -1
		if(LD_flood_visible.length === 0){
		  LD_flood_visible.push(-1);
		}
	...
	function f_layer_list_eris_update() {
		...
		dojo.forEach(inputs,function(input){
		  if (input.checked) {
			  LD_eris_visible.push(input.id.replace('m_layer_',''));
		  }
		});

		//if there aren't any layers visible set the array to be -1
		if(LD_eris_visible.length === 0){
		  LD_eris_visible.push(-1);
		}
	...
	function f_toc_vis(show){
		...
		if(show){
			dojo.forEach(groups,function(group){dojo.style(group,'display','block');});
		}
		else{
			dojo.forEach(groups,function(group){dojo.style(group,'display','none');});
		}
	}
	...
	function f_imagery_list_build(layer_selected){

		var selected = layer_selected;

		dojo.forEach(imagery_layers_json, function(img_lyr,index){
	...
	function f_map_identify_exec(click_evt) {
        ...
        //M_meri.infoWindow.setFeatures([ deferred ]);
			...
			case 'flash':
				for(var x=0; x<graphics_layer.graphics.length;x++){
					if(graphics_layer.graphics[x].attributes[object_attr] == oid){
						...
						var flashSymbol = new esri.symbol.SimpleFillSymbol(esri.symbol.SimpleFillSymbol.STYLE_SOLID,
	...
	function f_process_results_parcel(type,results){
		...
		//dojo.byId(target_div).innerHTML = '';
		...
		if(type=='buffer'){
			...
			e_export.setAttribute('style','padding-top:6px');
			
			dojo.byId("dResultsSummary").appendChild(e_export);
		}
	}
	...
	function f_query_owner_parcels_results(featureSets){
		...
		for(featureSet in featureSets){
			...
			for (var i=0, il=featureSets[featureSet].features.length; i<il; i++) {
				...
				// show all Parcel Attributes..
				// show link to parcel layer
	...
	function f_query_parcel_exec(){
		...
		if(dojo.byId("rdo_landuse_searchSelect").checked)
		{
			...
			var landuse_search = "LANDUSE_CODE  IN ("; 
...
/////////////////
	...
	function f_search_landuse_build(){
		...
		dojo.forEach(landuse_json, function(landuse,index){
			...
			e_lbl.innerHTML = landuse.lclass;

			// add the div and the label to the container
			e_div_container.appendChild(e_chk);
			e_div_container.appendChild(e_lbl);

			e_div_container.setAttribute('title',landuse.lclass);
	...
	function tabs_toggle(id){
		...
		var tabs = ["LayersToggle", "SearchTab", "ResultsTab", "HelpTab"];
		var selectedTab;
		
		for(var i in panes){
			 hide_visibility(panes[i]);	
		}
		show_visibility(id);
	...
	function resize_content_pane() {
		...
		for(var i in tabs)
		{
			dojo.style(tabs[i],'height', DivHeight + 'px');
		}	
	}
	...
	function f_summarize_totals(graphics){
		...
		//var summaryDiv = new dojo.doc.createElement('div');
		//summaryDiv.setAttribute('class','summary acres');
		//summaryDiv.innerHTML = field + ': ' + totalAcres;
				
		dojo.byId('dResultsSummary').innerHTML = graphics.length + ' parcels selected for a total of '+ Math.round(totalAcres*100)/100 + ' acres.';
	}
	...
	function dOn(el){
		dojo.style(dojo.byId(el),'display','block');
	}
	...
	function init() {
		...
		// load the municipality and qualifier search filter checkboxs
		...
		// add default imagery basemap
		IL_basemap = new esri.layers.ArcGISImageServiceLayer(DynamicLayerHost + "/ArcGIS/rest/services/Imagery/IMG_2009_C/ImageServer");

		M_meri.addLayer(IL_basemap);
		
		//var img_layers = ['IMG_2001_C','IMG_1930_BW','IMG_1958_BW','IMG_1969_BW','IMG_1978_BW','IMG_1985_BW','IMG_1992_BW','IMG_1995-97_CIR','IMG_2002_CIR','IMG_2008_C','IMG_2009_C','IMG_2002_BW'];

        //non cached NJMC parcels layer - no imagery
        ...
        //esriConfig.defaults.map.slider = { right:"-100px", bottom:"0px" };
		...
		// connect all map events to the appropriate functions
		// map single clicks are routed through the event handler when true navigation is not activated
		dojo.connect(M_meri, "onClick", f_map_click_handler);

        // after the layer is loaded, build the layer list
        dojo.connect(LD_base, "onLoad", f_layer_list_build);

		// after the imagery layer is loaded, build the imagery list
		dojo.connect(IL_basemap, "onLoad", f_imagery_list_build);
		...
		// these event handlers can be moved to functions for easier maintenance
